...
public class ProjectPropertyPage extends AbstractPropertyPage {
	...
	public boolean performOk() {
		try {
			if (storePreferences()) {
				boolean enabled = new ResourceSelector(
						getResource().getProject()).allowVisitProject();
				setBuilderEnabled(enabled);
				triggerRebuild();
			}
		} catch (CoreException exception) {
			String message = "Failed to store settings";
			Activator.handleError(message, exception);
			return false;
		}
		return true;
	}
	...
	protected Control createContents(Composite parent) {
		...
		includesView = new IncludesView(composite, SWT.NONE,
